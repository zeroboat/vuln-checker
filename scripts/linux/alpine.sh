...
check_alpine() {
    ...
    if command_exists apk; then
        ...
        append_log "업데이트 가능한 패키지: $(apk list --upgradable 2>/dev/null | wc -l)"
    fi
    ...
    # 계정관리 검사
    check_account_management_alpine
...
check_account_management_alpine() {
    ...
    local system_users=("root" "bin" "sys" "sync" "games" "man" "lp" "mail" "news" "uucp" "proxy" "list" "irc" "gnats" "nobody" "sshd")
